...
const LoginModal = ({ open, onClose, onSwitchToRegister, onAdminOpen }: Props) => {
  ...
  useEffect(() => {
    if (!open) { setView("login"); setError(""); setEmail(""); setPassword(""); setResetEmail(""); }
  }, [open]);
  ...
  const handleForgot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err) setError("Email introuvable ou erreur, réessaie.");
        ...
        <div className="px-8 pt-8 pb-8">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => { setResetEmail(email); setView("forgot"); setError(""); }} className="text-xs text-garden-blue-dark hover:underline">
                      Mot de passe oublié ?
                    </button>
                  </div>
                  <div className="relative">
          ...
          {view === "forgot" && (
            <>
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h2 className="text-xl font-black tracking-tight text-foreground">Mot de passe oublié</h2>
                  <p className="text-sm text-muted-foreground mt-1">On vous envoie un lien de réinitialisation</p>
                </div>
                <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-muted transition-colors text-muted-foreground hover:text-foreground -mt-1">
                  <X className="w-4 h-4" />
                </button>
              ...
              <p className="text-center text-sm text-muted-foreground mt-5">
                <button onClick={() => { setView("login"); setError(""); }} className="font-semibold text-garden-blue-dark hover:underline">
          ...
          {view === "forgot_sent" && (
            <div className="flex flex-col items-center text-center gap-4 py-4">
              ...
              <div>
                <h2 className="text-xl font-black tracking-tight text-foreground">Email envoyé !</h2>
